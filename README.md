...
Notice that the proposed loss function  has a lower minimum
Results:

After 20 epochs of training with each loss with a 4 layered MLP (+Dropout), we see that the Potential loss has slightly better performance on the test set, scoring 98.10% accuracy against 97.37% with the Contrastive loss. We have used batch size=128. It is interesting to see the distances between the embedding vectors after training. Here we show the distributions of distances between positive and negative pairs on the test set:
<p align="center">
